...
depends=('shinken')
makedepends=('curl' 'bc')
backup=('var/lib/shinken/etc/modules/webui2.cfg')
...
package() {
  ...
  mkdir -p $pkgdir/var/lib/shinken/etc/modules
  mv etc/modules/$_moddir.cfg $pkgdir/var/lib/shinken/etc/modules/
  chmod 0640 $pkgdir/var/lib/shinken/etc/modules/$_moddir.cfg
}
